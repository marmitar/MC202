...
pub(super) unsafe trait FieldTuple {
    ...
    // unsafe fn layouts<const N: usize>() ->
}

/// Build the layout for the equivalent `#[repr(C)]` struct.
#[inline]
pub(super) const fn layout<T: FieldTuple>(val: &T::Last) -> Layout {
    match T::START_LAYOUT.extend(Layout::for_value(val)) {
